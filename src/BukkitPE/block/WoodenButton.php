...
use BukkitPE\level\sound\ButtonClickSound;
use BukkitPE\level\sound\ButtonReturnSound;
use BukkitPE\Player;
...
class WoodenButton extends Flowable implements Redstone, RedstoneSource, RedstoneSwitch{
	...
	public function onUpdate($type){
		...
		elseif($type === Level::BLOCK_UPDATE_NORMAL){
			...
			if($this->getSide($pb)->isTransparent() === true && !($pb === 0 && ($this->getSide(0) instanceof Slab && ($this->getSide(0)->meta & 0x08) === 0x08) || ($this->getSide(0) instanceof WoodSlab && ($this->getSide(0)->meta & 0x08) === 0x08) || ($this->getSide(0) instanceof Stair && ($this->getSide(0)->meta & 0x04) === 0x04))){
	...
	public function place(Item $item, Block $block, Block $target, $face, $fx, $fy, $fz, Player $player = null){
		if(!$target->isTransparent() || ($face === 0 && ($this->getSide(0) instanceof Slab && ($this->getSide(0)->meta & 0x08) === 0x08) || ($this->getSide(0) instanceof WoodSlab && ($this->getSide(0)->meta & 0x08) === 0x08) || ($this->getSide(0) instanceof Stair && ($this->getSide(0)->meta & 0x04) === 0x04))){
			$this->meta = $face;

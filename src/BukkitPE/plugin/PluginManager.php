...
use BukkitPE\command\PluginCommand;
use BukkitPE\command\SimpleCommandMap;
use BukkitPE\event\Event;
use BukkitPE\event\EventPriority;
...
use BukkitPE\permission\Permission;
use BukkitPE\Server;
...
class PluginManager{
	...
	protected function parseYamlCommands(Plugin $plugin){
		...
		foreach($plugin->getDescription()->getCommands() as $key => $data){
			if(strpos($key, ":") !== false){
				$this->server->getLogger()->critical($this->server->getLanguage()->translateString("BukkitPE.plugin.commandError", [$key, $plugin->getDescription()->getFullName()]));
				continue;
			}
			if(is_array($data)){
				...
				if(isset($data["aliases"]) and is_array($data["aliases"])){
					...
					foreach($data["aliases"] as $alias){
						if(strpos($alias, ":") !== false){
							$this->server->getLogger()->critical($this->server->getLanguage()->translateString("BukkitPE.plugin.aliasError", [$alias, $plugin->getDescription()->getFullName()]));
							continue;
						}
						$aliasList[] = $alias;
					}
		...
		foreach($event->getHandlers()->getRegisteredListeners() as $registration){
			...
			try{
				...
				$this->server->getLogger()->critical(
					$this->server->getLanguage()->translateString("BukkitPE.plugin.eventError", [
						$event->getEventName(),
			...
			if(!$method->isStatic()){
				...
				if(count($parameters) === 1 and $parameters[0]->getClass() instanceof \ReflectionClass and is_subclass_of($parameters[0]->getClass()->getName(), Event::class)){
					...
					if(strpos((string) $reflection->getDocComment(), "@deprecated") !== false and $this->server->getProperty("settings.deprecated-verbose", true)){
						$this->server->getLogger()->warning($this->server->getLanguage()->translateString("BukkitPE.plugin.deprecatedEvent", [
							$plugin->getName(),
							$class,

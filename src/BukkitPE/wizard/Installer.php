...
class Installer{
	...
	private function showLicense(){
		echo $this->lang->welcome_to_BukkitPE . "\n";
		echo <<<LICENSE

  BukkitPE is run under the Creative Commons Attribution-NonCommercial-NoDerivatives 4.0. 
...
LICENSE;
	...
	private function generateUserFiles(){
		...
		if($op === ""){
			...
			$ops = new Config(\BukkitPE\DATA . "ops.txt", Config::ENUM);
			$ops->set($op, true);
			$ops->save();
		}
	...
	private function endWizard(){
		...
		echo "[*] " . $this->lang->BukkitPE_plugins . "\n";
		echo "[*] " . $this->lang->BukkitPE_will_start . "\n\n\n";
		sleep(4);
	}

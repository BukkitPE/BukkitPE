...
class CrashDump{
	...
	public function __construct(Server $server){
		$this->time = time();
		$this->server = $server;
		$this->path = $this->server->getDataPath() . "CrashDump_" . date("D_M_j-H.i.s-T_Y", $this->time) . ".txt";
	...
	private function pluginsData(){
		if(class_exists("BukkitPE\\plugin\\PluginManager", false)){
			$this->addLine();
	...
	private function extraData(){
		...
		if($this->server->getProperty("auto-report.send-settings", true) !== false){
			...
			$this->data["BukkitPE.yml"] = @file_get_contents($this->server->getDataPath() . "BukkitPE.yml");
		}else{
			$this->data["BukkitPE.yml"] = "";
	...
	private function baseCrash(){
		...
		if(strpos($error["file"], "src/BukkitPE/") === false and strpos($error["file"], "src/raklib/") === false and file_exists($error["fullFile"])){
			$this->addLine();
			...
			foreach($this->server->getPluginManager()->getPlugins() as $plugin){
				$filePath = \BukkitPE\cleanPath($file->getValue($plugin));

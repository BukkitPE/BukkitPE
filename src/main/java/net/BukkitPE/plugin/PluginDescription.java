 ...
 * load: POSTWORLD
 * author: 粉鞋大妈
 * description: A simple Hello World plugin for BukkitPE
 ...
 * 要让BukkitPE能够正常加载你的插件，必须要合理地填写这几个字段。<br>
 ...
 * your plugin can be normally loaded by BukkitPE.<br>
 * </p>
 * <p>
 * <p>接下来对所有的字段做一些说明，<b>加粗</b>的字段表示必需，<i>斜体</i>表示可选：（来自
 * <a href="http://www.cnblogs.com/xtypr/p/BukkitPE_plugin_start_from_0_about_config.html">粉鞋大妈的博客文章</a>）<br>
 * Here are some instructions for there items, <b>bold</b> means required, <i>italic</i> means optional: (From
 * <a href="http://www.cnblogs.com/xtypr/p/BukkitPE_plugin_start_from_0_about_config.html">a blog article of @粉鞋大妈</a>)
 * <ul>
 * <li><b>name</b><br>
 * 字符串，表示这个插件的名字，名字是区分不同插件的标准之一。
 * 插件的名字<i>不能包含“BukkitPE”“minecraft”“mojang”</i>这几个字符串，而且不应该包含空格。<br>
 * String, the plugin name. Name is one of the ways to distinguish different plugins.
 * A plugin name <i>can't contain 'BukkitPE' 'minecraft' 'mojang'</i>, and shouldn't contain spaces.</li>
 * <li><b>version</b><br>
 ...
 * <li><b>api</b><br>
 * 字符串序列，表示这个插件支持的BukkitPE API版本号列表。插件作者应该调试能支持的API，然后把版本号添加到这个列表。<br>
 * A set of String, the BukkitPE API versions that the plugin supports. Plugin developers should debug in different
 * BukkitPE APIs and try out the versions supported, and add them to this list. </li>
 * <li><b>main</b><br>
 * 字符串，表示这个插件的主类。插件的主类<i>不能放在“net.BukkitPE”包下</i>。<br>
 * String, the main class of plugin. The main class<i> can't be placed at 'net.BukkitPE' package</i>.</li>
...
public class PluginDescription {
    ...
    private void loadMap(Map<String, Object> plugin) throws PluginException {
        ...
        if (this.main.startsWith("net.BukkitPE.")) {
            throw new PluginException("Invalid PluginDescription main, cannot start within the net.BukkitPE. package");
        }
    ...
    /**
     * 返回这个插件支持的BukkitPE API版本列表。<br>
     * Returns all BukkitPE API versions this plugin supports.
     *
     * @return 这个插件支持的BukkitPE API版本列表。<br>A list of all BukkitPE API versions String this plugin supports.
     * @see PluginDescription
     * @since BukkitPE 1.0 | BukkitPE API 1.0.0
     */
     ...
     * <p>
     * <p>BukkitPE插件的依赖有这些注意事项：<br>Here are some note for BukkitPE plugin depending:
     * <ul>
     ...
     * If a plugin relies on another one, the another one must be installed at the same time, or BukkitPE
     * won't load this plugin.</li>
     * <li>当一个插件所依赖的插件不存在时，BukkitPE不会加载这个插件，但是会提醒用户去安装所依赖的插件。<br>
     * When the required dependency plugin does not exists, BukkitPE won't load this plugin, but will tell the
     * user that this dependency is required.</li>
     * </ul></p>
     * <p>
     * <p>举个例子，如果A插件依赖于B插件，在没有安装B插件而安装A插件的情况下，BukkitPE会阻止A插件的加载。
     * 只有在安装B插件前安装了它所依赖的A插件，BukkitPE才会允许加载B插件。<br>
     * For example, there is a Plugin A which relies on Plugin B. If you installed A without installing B,
     * BukkitPE won't load A because its dependency B is lost. Only when B is installed, A will be loaded
     * by BukkitPE.</p>
     *
     ...
     * The load action of a BukkitPE plugin begins from main class. The name of main class should be defined
    ...
    /**
     * @return 这个插件是否有BukkitPE团队颁发的安全证书。
     */

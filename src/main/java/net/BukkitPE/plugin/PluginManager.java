...
public class PluginManager {
                ...
                for (File file : dictionary.listFiles(new FilenameFilter() {
                    ...
                    try {
                        ...
                        if (description != null) {
                            ...
                            if (name.toLowerCase().contains("BukkitPE") || name.toLowerCase().contains("minecraft") || name.toLowerCase().contains("mojang")) {
                                this.server.getLogger().error(this.server.getLanguage().translateString("BukkitPE.plugin.loadError", new String[]{name, "%BukkitPE.plugin.restrictedName"}));
                                continue;
                            ...
                            for (String version : description.getCompatibleAPIs()) {
                                ...
                                if (!Pattern.matches("[0-9]\\.[0-9]\\.[0-9]", version)) {
                                    this.server.getLogger().error(this.server.getLanguage().translateString("BukkitPE.plugin.loadError", new String[]{name, "Wrong API format"}));
                                    continue;
                                }
                            ...
                            if (!compatible) {
                                this.server.getLogger().error(this.server.getLanguage().translateString("BukkitPE.plugin.loadError", new String[]{name, "%BukkitPE.plugin.incompatibleAPI"}));
                            }
                        ...
                        this.server.getLogger().error(this.server.getLanguage().translateString("BukkitPE.plugin" +
                    ...
                    if (dependencies.containsKey(name)) {
                        for (String dependency : new ArrayList<>(dependencies.get(name))) {
                            if (loadedPlugins.containsKey(dependency) || this.getPlugin(dependency) != null) {
                                ...
                                this.server.getLogger().critical(this.server.getLanguage().translateString("BukkitPE" +
                                        ".plugin.loadError", new String[]{name, "%BukkitPE.plugin.unknownDependency"}));
                                break;
                            }
                        }
                ...
                if (missingDependency) {
                    ...
                    if (missingDependency) {
                        for (String name : plugins.keySet()) {
                            this.server.getLogger().critical(this.server.getLanguage().translateString("BukkitPE.plugin.loadError", new String[]{name, "%BukkitPE.plugin.circularDependency"}));
                        }
                        plugins.clear();
                    }
                }
    ...
    protected List<PluginCommand> parseYamlCommands(Plugin plugin) {
        ...
        for (Map.Entry entry : plugin.getDescription().getCommands().entrySet()) {
            ...
            if (key.contains(":")) {
                this.server.getLogger().critical(this.server.getLanguage().translateString("BukkitPE.plugin.commandError", new String[]{key, plugin.getDescription().getFullName()}));
                continue;
            }
                ...
                if (((Map) data).containsKey("aliases")) {
                    ...
                    if (aliases instanceof List) {
                        ...
                        for (String alias : (List<String>) aliases) {
                            if (alias.contains(":")) {
                                this.server.getLogger().critical(this.server.getLanguage().translateString("BukkitPE.plugin.aliasError", new String[]{alias, plugin.getDescription().getFullName()}));
                                continue;
                            }
                            aliasList.add(alias);
                        }
            ...
            for (Class<?> clazz = eventClass; Event.class.isAssignableFrom(clazz); clazz = clazz.getSuperclass()) {
                ...
                if (clazz.getAnnotation(Deprecated.class) != null) {
                    if (Boolean.valueOf(String.valueOf(this.server.getConfig("settings.deprecated-verbpse", true)))) {
                        this.server.getLogger().warning(this.server.getLanguage().translateString("BukkitPE.plugin.deprecatedEvent", new String[]{plugin.getName(), clazz.getName(), listener.getClass().getName() + "." + method.getName() + "()"}));
                    }
                    break;
                }
            }

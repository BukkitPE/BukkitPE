...
*/
package net.BukkitPE.entity.ai;
import net.BukkitPE.Player;
import net.BukkitPE.Server;
import net.BukkitPE.entity.Animal;
import net.BukkitPE.entity.Chicken;
import net.BukkitPE.entity.Cow;
import net.BukkitPE.entity.Creeper;
import net.BukkitPE.entity.Entity;
import net.BukkitPE.entity.IronGolem;
import net.BukkitPE.entity.Monster;
import net.BukkitPE.entity.Mooshroom;
import net.BukkitPE.entity.Ocelot;
import net.BukkitPE.entity.Pig;
import net.BukkitPE.entity.PigZombie;
import net.BukkitPE.entity.Sheep;
import net.BukkitPE.entity.Skeleton;
import net.BukkitPE.entity.SnowGolem;
import net.BukkitPE.entity.Wolf;
import net.BukkitPE.entity.Zombie;
...
import net.BukkitPE.event.entity.EntityGenerateEvent;
...
import net.BukkitPE.scheduler.CallbackTask;
...
public class AIHolder{
	...
	public static void __construct(Server server){
		this.server = server;
	 ...
	 * 设置僵尸仇恨半径
	 */
	public void setZombieHatred_r(r){
	 ...
	 * 设置夜晚刷僵尸范围（以每个玩家为中心）
	 */
	public void setZombieBirth_r(r){
		this.birth_r = r;
	}
	/**
	 * @param v
	 ...
	 * （可用于更改刷怪时间间隔）
	 */
	public boolean RestartSpawnTimer(tick = 600){
	 ...
	 * 停止刷怪计时器
	 */
	public boolean CancelSpawnTimer(){
		task = this.tasks['ZombieGenerate'];
		if(task instanceof TaskHandler){
			task.cancel();
			return true;
	 ...
	 *                            出生一只僵尸在某坐标
	 */
	public void spawnZombie(Position pos, maxHealth = 20, health = 20){
		this.getZombie(pos, maxHealth, health).spawnToAll();
		//this.getLogger().info("生成了一只僵尸");
	}
	public Zombie getZombie(Position pos, maxHealth = 20, health = 20){
	 ...
	 *                            出生一只苦力怕在某坐标
	 */
	public void spawnCreeper(Position pos, maxHealth = 20, health = 20){
		chunk = pos.level.getChunk(pos.x >> 4, pos.z >> 4, false);
		nbt = this.getNBT();
	 ...
	 *                            出生一只骷髅弓箭手在某坐标
	 */
	public void spawnSkeleton(Position pos, maxHealth = 20, health = 20){
		chunk = pos.level.getChunk(pos.x >> 4, pos.z >> 4, false);
		nbt = this.getNBT();
	 ...
	 * @return Cow
	 */
	public CowgetCow(Position pos, maxHealth = 20, health = 20){
		chunk = pos.level.getChunk(pos.x >> 4, pos.z >> 4, false);
		nbt = this.getNBT();
	 ...
	 *                            出生一只豬在某坐标
	 */
	public void spawnPig(Position pos, maxHealth = 20, health = 20){
		chunk = pos.level.getChunk(pos.x >> 4, pos.z >> 4, false);
		nbt = this.getNBT();
	 ...
	 *                            出生一只羊在某坐标
	 */
	public void spawnSheep(Position pos, maxHealth = 20, health = 20){
		chunk = pos.level.getChunk(pos.x >> 4, pos.z >> 4, false);
		nbt = this.getNBT();
	 ...
	 *                            出生一只雞在某坐标
	 */
	public void spawnChicken(Position pos, maxHealth = 20, health = 20){
		chunk = pos.level.getChunk(pos.x >> 4, pos.z >> 4, false);
		nbt = this.getNBT();
	 ...
	 * 根据僵尸血量获取对应攻击值
	 */
	public Int getZombieDamage(zoHealth){
	...
	public Int getSkeletonDamage(zoHealth){
		dif = this.getServer().getDifficulty();
		switch(dif){
			case 0:
				return 0;
	 ...
	 * 根据玩家的装备获取玩家应受到的伤害值
	 */
	public Int getPlayerDamage(Player player, damage){
		armorValues = [
			Item::LEATHER_CAP => 1,
	 ...
	 * 获取某坐标(位置)的亮度
	 */
	public Int getLight(Position pos){
	 ...
	 * 获取yaw角度
	 */
	public Int getyaw(mx, mz){  //根据motion计算转向角度
		//转向计算
		if(mz == 0){  //斜率不存在
			if(mx < 0){
				yaw = -90;
	 ...
	 * 获取pitch角度
	 */
	public Int getpitch(Vector3 from, Vector3 to){
	 ...
	 * 并给出原因
	 */
	public Int ifjump(Level level, Vector3 v3, hate = false, reason = false){  //boybook Y轴算法核心函数
		x = floor(v3.getX());
		y = floor(v3.getY());
		z = floor(v3.getZ());
		//echo (y." ");
	...
	public String whatBlock(Level level, v3){  //boybook的y轴判断法 核心 什么方块？
				...
				if(this.whatBlock(level, v3) == "block"){
					...
					if(level.getBlock(v3_1).getID() == 0 and level.getBlock(v3_2).getID() == 0){  //找到地面
						...
						if(count(monsters) > max){
							...
							this.server.getPluginManager().callEvent(ev = new EntityGenerateEvent(pos, mobId, EntityGenerateEvent::CAimport_AI_HOLDER));
							if(!ev.isCancelled()){
	...
	public void EntityDamage(EntityDamageEvent event){  //击退修复
	...
	public void knockBackover(Entity entity, Vector3 v3){
		if(entity instanceof Entity){
			if(this.zombie.contains(entity.getId())){
				entity.setPosition(v3);
				this.zombie.contains(entity.getId()).replace(["knockBack",false);
			}
			if(this.Cow.contains(entity.getId())){
				entity.setPosition(v3);
				this.Cow.contains(entity.getId()).replace(["knockBack",false);
			}
			if(this.Pig.contains(entity.getId())){
				entity.setPosition(v3);
				this.Pig.contains(entity.getId()).replace(["knockBack",false);
			}
			if(this.Sheep.contains(entity.getId())){
				entity.setPosition(v3);
				this.Sheep.contains(entity.getId()).replace(["knockBack",false);
			}
			if(this.Chicken.contains(entity.getId())){
				entity.setPosition(v3);
				this.Chicken.contains(entity.getId()).replace(["knockBack",false);
			}
			if(this.Skeleton.contains(entity.getId())){
				entity.setPosition(v3);
				this.Skeleton.contains(entity.getId()).replace(["knockBack",false);
			}
			if(this.Creeper.contains(entity.getId())){
				entity.setPosition(v3);
				this.Creeper.contains(entity.getId()).replace(["knockBack",false);
			}
		}
	}
}

...
public interface CommandExecutor {
     ...
     * <p>
     * <p>关于返回值，如果返回{@code false}，BukkitPE会给sender发送这个命令的使用方法等信息，来表示这个命令没有使用成功。
     * 如果你的命令成功的发挥了作用，你应该返回{@code true}来表示这个命令已执行成功。<br>
     * If this function returns {@code false}, BukkitPE will send command usages to command sender, to explain that
     ...
     * 可以使用{@link net.BukkitPE.command.Command#testPermissionSilent}。<br>
     * If you want to test whether a command sender has the permission to execute a command,
     * you can use {@link net.BukkitPE.command.Command#testPermissionSilent}.</p>
     *

...
import static net.BukkitPE.timings.TimingsManager.HISTORY;
...
public class TimingsExport extends Thread {
    ...
    public static void reportTimings(CommandSender sender) {
        ...
        if (!Timings.getIgnoredConfigSections().contains("all")) {
            JsonObject BukkitPE = JsonUtil.toObject(Server.getInstance().getConfig().getRootSection());
            Timings.getIgnoredConfigSections().forEach(BukkitPE::remove);
            config.add("BukkitPE", BukkitPE);
        } else {
            config.add("BukkitPE", null);
        }
    ...
    public synchronized void start() {
        if (this.sender instanceof RemoteConsoleCommandSender) {
            this.sender.sendMessage(new TranslationContainer("BukkitPE.command.timings.rcon"));
            run();
        } else {
            super.start();
        }
    ...
    @Override
    public void run() {
        this.sender.sendMessage(new TranslationContainer("BukkitPE.command.timings.uploadStart"));
        ...
        try {
            ...
            con.setRequestProperty("User-Agent", "Sponge/" + Server.getInstance().getName() + "/" + InetAddress.getLocalHost().getHostName()); //It will be Sponge unless BukkitPE is added on Aikar's parser side
            ...
            this.sender.sendMessage(new TranslationContainer("BukkitPE.command.timings.timingsLocation", location));
            if (!(this.sender instanceof ConsoleCommandSender)) {
                Server.getInstance().getLogger().info(Server.getInstance().getLanguage().translateString("BukkitPE.command.timings.timingsLocation", location));
            }

            if (response != null && !response.isEmpty()) {
                Server.getInstance().getLogger().info(Server.getInstance().getLanguage().translateString("BukkitPE.command.timings.timingsResponse", response));
            }
            ...
            writer.write(Server.getInstance().getLanguage().translateString("BukkitPE.command.timings.timingsLocation", location) + "\n\n");
            writer.write(new GsonBuilder().setPrettyPrinting().create().toJson(this.out));
            writer.close();

            Server.getInstance().getLogger().info(Server.getInstance().getLanguage().translateString("BukkitPE.command.timings.timingsWrite", fileName));
        } catch (IOException exception) {
            this.sender.sendMessage(TextFormat.RED + new TranslationContainer("BukkitPE.command.timings.reportError"));
            if (response != null) {
                Server.getInstance().getLogger().alert(response);
            }
